...
// Parts are printed in the order in which they were entered ino the database.
void print(void)
{
    int i;
...
void selection_sort(void)
{
    ...
    for (i = 0; i < num_parts-1; i++)
    {
        ...
        {
            if (inventory[j].number < inventory[min_index].number)
                min_index = j;
        }

        swap_parts(i, min_index);
    }
}
...
void swap_parts(int index1, int index2)
{
    ...
    int number = inventory[index1].number;
    char *name = inventory[index1].name;
    int on_hand = inventory[index1].on_hand;

    inventory[index1].number = inventory[index2].number;
    strcpy(inventory[index1].name, inventory[index2].name);
    inventory[index1].on_hand = inventory[index2].on_hand;

    inventory[index2].number = number;
    strcpy(inventory[index2].name, name);
    inventory[index2].on_hand = on_hand;
}