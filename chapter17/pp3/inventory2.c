...
// Prints an error message if the user enters and illegal code.
int main()
{
    char code;
...
// Returns a pointer to the node containing the part number; if the part number is not found, returns NULL.
struct part *find_part(int number)
{
    struct part *p;
...
// Prompts the user for information about a new part and then inserts the part into the inventory list; the list remaains sorted by part number.
// Prints an error message and returns prematurely if the part already exists or space could not be allocated for the part.
void insert(void)
{
...
void erase(void)
{
    ...
    printf("Enter the part to remove: ");
    scanf("%d", &number);
